...
export default function TransactionTable({ rows = [], onBulkAdd }) {
  ...
  const categories = useMemo(() => {
    const set = new Set(
      rows.map((r) => (r.category || "Uncategorized").trim() || "Uncategorized")
    );
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((tx) => {
      ...
      if (debouncedQ) {
        ...
        if (
          !merchant.includes(debouncedQ) &&
          !comment.includes(debouncedQ) &&
          !catLower.includes(debouncedQ)
        ) {
          return false;
        }
      }
    ...
    try {
      ...
      for (const r of rows) {
        const date = normalizeDate(
          r.Date || r.date || r["Transaction Date"] || ""
        );
        ...
        const amountDec = parseFloat(
          String(r.Amount ?? r.amount ?? "0").replace(/[, ]/g, "")
        );
        const amountCents = Number.isFinite(amountDec)
          ? Math.round(amountDec * 100)
          : 0;
          ...
          {importErr ? (
            ...
            <>
              <div className="text-slate-500 mb-2">
                Previewing first {importPreview.length} of {importCount} parsed
                rows
              </div>
